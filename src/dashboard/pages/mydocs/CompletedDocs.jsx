    ...
    useEffect(() => {
        const fetchCompletedDocs = async (query) => {
            ...
            try {
                let response;
                if (query && query.trim() !== "") {
                    response = await signerApi.getSearchCompletedDocumentsByEmail(userEmail, query);
                } else {
                                    ...
                                    <td>
                                        <div style={{ display: 'flex', gap: '8px' }}>
                                            <Button
                                                ...
                                                size="sm"
                                                className="me-2"
                                                onClick={() => handleView(doc.documentId, doc.documentName, doc.signedFile, "completed")}
                                                title="View"
                                            >
                                                <FaEye />
                                            </Button>
